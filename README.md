...
- [x] (NEW!) Локализация языка бота
  - Доступные языки :gb: :de: :ru: :tr: :it: :es: :indonesia: :netherlands: :cn: :vietnam:
...
| `VOICE_REPLY_PROMPTS`              | Список фраз, разделенных точкой с запятой ("Hi bot;Hello chat"). Если стенограмма начинается с любой из них - она будет рассматриваться как подсказка | -
...
| `BOT_LANGUAGE`                     | Язык общих сообщений бота. В настоящее время доступны: `en`, `de`, `ru`, `tr`, `it`, `es`, `id`, `nl`, `cn`, `vi`.  [Contribute with additional translations](https://github.com/n3d1117/chatgpt-telegram-bot/discussions/219) | `ru`                               |
